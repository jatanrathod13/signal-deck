...
function addResult(name: string, passed: boolean, latency?: number, error?: string) {
...
async function waitForEvent(socket: Socket, eventName: string, timeout: number): Promise<unknown> {
...
async function runTests() {
  ...
  try {
    ...
    addResult('Agent Status Update (Socket.IO)', latency <= 2000, latency,
      latency > 2000 ? 'Exceeds 2 second threshold' : undefined);
  } catch (error) {
    const errorMsg = error instanceof Error ? error.message : String(error);
  ...
  try {
    ...
    const agentData = await agentResponse.json() as { data: { id: string } };
    ...
    addResult('Task Status Update (Socket.IO)', latency <= 2000, latency,
      latency > 2000 ? 'Exceeds 2 second threshold' : undefined);
  } catch (error) {
    const errorMsg = error instanceof Error ? error.message : String(error);
  ...
  try {
    ...
    addResult('Real-time Events (No Polling)', passed, undefined,
      passed ? undefined : 'No real-time events received');
  } catch (error) {
    const errorMsg = error instanceof Error ? error.message : String(error);
  ...
  addResult('No Socket Errors', !hasSocketErrors, undefined,
    hasSocketErrors ? 'Socket errors detected' : undefined);
